# rental-suite-manager

> Manage smarter, rent easier.

跨平台套房管理系統，支援 macOS、Windows、Web、Android、iOS。

---

## 目錄

- [專案資訊](#專案資訊)
- [S-01 專案基礎建設與跨平台架構](#s-01-專案基礎建設與跨平台架構)
- [S-02 房東帳號與身份驗證](#s-02-房東帳號與身份驗證)
- [S-03 房源管理套房-crud](#s-03-房源管理套房-crud)
- [S-04 租客管理](#s-04-租客管理)
- [S-05 租約合約管理](#s-05-租約合約管理)
- [S-06 租金收款與帳務管理](#s-06-租金收款與帳務管理)
- [S-07 水電錶抄表管理](#s-07-水電錶抄表管理)
- [S-08 維修報修管理](#s-08-維修報修管理)
- [S-09 儀表板與數據總覽](#s-09-儀表板與數據總覽)
- [S-10 通知與推播系統](#s-10-通知與推播系統)

---

## 專案資訊

| 項目 | 內容 |
|------|------|
| Repo | `rental-suite-manager` |
| 副標題 | Manage smarter, rent easier. |
| 支援平台 | macOS、Windows、Web、Android、iOS |
| Stories | 10 |
| Tasks | 57 |

---

## S-01 專案基礎建設與跨平台架構

**目標**：建立整個專案的技術底座，確保各平台可以共用程式碼並獨立部署。

| Task | 說明 | 類型 | 狀態 |
|------|------|------|------|
| T-01 | 建立 Monorepo 結構（Turborepo），規劃 `apps/` 與 `packages/` 目錄 | infra | ✅ Done |
| T-02a | 設定 Flutter 跨平台框架，驗證 macOS 建置與執行 | FE | ✅ Done |
| T-02b | 驗證 Windows 建置與執行 | FE | |
| T-02c | 驗證 Android 建置與執行 | FE | |
| T-02d | 驗證 iOS 建置與執行 | FE | |
| T-03 | 建立 Web 前端專案（Next.js），配置 Tailwind CSS | FE | |
| T-04 | 設定後端 API 服務（Node.js + Fastify），定義 REST 規範 | BE | ✅ Done |
| T-05 | 建立 CI/CD Pipeline（GitHub Actions），自動化各平台建置與測試 | infra | |
| T-06 | 設定共用設計 Token（Design System），統一色彩、字體、間距規範 | design | ✅ Done |

---

## S-02 房東帳號與身份驗證

**目標**：提供安全的帳號系統，支援多種登入方式與多裝置同步。

| Task | 說明 | 類型 |
|------|------|------|
| T-07 | 實作 Email / 密碼註冊與登入流程 | BE |
| T-08 | 整合 Google / Apple SSO 第三方登入 | BE |
| T-09 | JWT Access Token + Refresh Token 管理機制 | BE |
| T-10 | 設計登入、忘記密碼、重設密碼畫面（各平台適配） | FE |
| T-11 | 多裝置同步登入狀態管理與登出處理 | FE |

---

## S-03 房源管理（套房 CRUD）

**目標**：讓房東可以完整管理名下所有套房，包含資訊、照片與平面圖。

| Task | 說明 | 類型 |
|------|------|------|
| T-12 | 設計房源資料模型（樓層、房號、坪數、設施、狀態） | BE |
| T-13 | 實作房源新增、編輯、刪除、封存 API | BE |
| T-14 | 房源列表頁面，支援篩選（狀態、樓層）與排序 | FE |
| T-15 | 房源詳細資訊頁面，顯示出租狀態與目前租客資訊 | FE |
| T-16 | 房源照片上傳（多張），整合雲端儲存（S3 / Cloudflare R2） | BE |
| T-17 | 建立樓層平面圖視覺化介面（2D 格局圖，可點擊各房間） | FE |
| T-18 | 批次匯入房源（CSV / Excel 格式支援） | BE |

---

## S-04 租客管理

**目標**：管理所有租客資料，記錄入退房歷史與相關文件。

| Task | 說明 | 類型 |
|------|------|------|
| T-19 | 設計租客資料模型（姓名、身份證、聯絡方式、入住日期） | BE |
| T-20 | 租客新增、編輯、刪除與搜尋 API | BE |
| T-21 | 租客列表與詳細資料頁面 | FE |
| T-22 | 租客入住、退房流程操作介面 | FE |
| T-23 | 上傳租客身份證件與合約文件（PDF 支援） | BE |
| T-24 | 租客歷史紀錄查詢（曾住哪間房、租期） | FE |

---

## S-05 租約合約管理

**目標**：數位化租約流程，支援模板、版本管理與到期提醒。

| Task | 說明 | 類型 |
|------|------|------|
| T-25 | 建立租約資料模型（租期、租金、押金、特殊條款） | BE |
| T-26 | 合約新增與版本管理 API | BE |
| T-27 | 合約模板系統，支援自訂欄位並自動填入租客／房源資料 | FE |
| T-28 | 合約到期提醒機制（在到期前 N 天自動推播 / Email） | BE |
| T-29 | 合約 PDF 匯出與列印功能 | FE |

---

## S-06 租金收款與帳務管理

**目標**：自動化租金追蹤，提供清楚的帳務報表與逾期通知。

| Task | 說明 | 類型 |
|------|------|------|
| T-30 | 建立帳務資料模型（應收租金、繳費紀錄、逾期狀態） | BE |
| T-31 | 每月自動產生應收帳款單 | BE |
| T-32 | 手動標記繳費（現金 / 轉帳）並上傳收款憑證 | FE |
| T-33 | 逾期未繳提醒通知（推播 + Email + SMS） | BE |
| T-34 | 月收入報表頁面（各房間收款狀況、總收入統計） | FE |
| T-35 | 帳務匯出（Excel / CSV）功能 | FE |
| T-36 | 水電費用分攤計算（公用電費按人頭或用量分攤） | BE |

---

## S-07 水電錶抄表管理

**目標**：數位化每月抄表流程，自動計算用量與應收費用。

| Task | 說明 | 類型 |
|------|------|------|
| T-37 | 建立水錶 / 電錶抄表紀錄資料模型 | BE |
| T-38 | 抄表輸入介面，支援手動輸入與相機掃描（OCR） | FE |
| T-39 | 自動計算用量差值與應收金額 | BE |
| T-40 | 抄表歷史紀錄與用量趨勢圖表 | FE |
| T-41 | 每月抄表提醒推播通知 | BE |

---

## S-08 維修報修管理

**目標**：集中管理房屋維修需求，追蹤進度與費用。

| Task | 說明 | 類型 |
|------|------|------|
| T-42 | 建立報修單資料模型（類別、描述、狀態、優先度） | BE |
| T-43 | 房東端報修單列表管理頁面（待處理 / 處理中 / 完成） | FE |
| T-44 | 維修費用紀錄與廠商資訊管理 | BE |
| T-45 | 報修照片上傳（前後對比） | FE |
| T-46 | 維修狀態更新推播通知（通知租客） | BE |

---

## S-09 儀表板與數據總覽

**目標**：提供房東一眼掌握整體出租狀況與財務健康度。

| Task | 說明 | 類型 |
|------|------|------|
| T-47 | 首頁儀表板：出租率、本月收入、逾期帳款、待處理報修 | FE |
| T-48 | 收入趨勢折線圖（月度 / 年度） | FE |
| T-49 | 房間狀態色塊圖（空房 / 出租中 / 維修中） | FE |
| T-50 | Dashboard 資料 API 端點（聚合查詢，效能優化） | BE |

---

## S-10 通知與推播系統

**目標**：確保重要事件即時送達，支援所有平台的通知管道。

| Task | 說明 | 類型 |
|------|------|------|
| T-51 | 整合 FCM（Firebase Cloud Messaging）支援 Android / iOS 推播 | BE |
| T-52 | macOS / Windows 桌面原生通知整合 | FE |
| T-53 | Email 通知整合（Resend / SendGrid） | BE |
| T-54 | 通知偏好設定頁面（各類通知開關） | FE |

---

*共 10 Stories、57 Tasks*